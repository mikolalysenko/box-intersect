...
function testOverlap(d, axis, flip, r, b) {
  var r0 = r[axis]
  var r1 = r[axis+d]
  var b0 = b[axis]
  if(b0 < r0 || r1 < b0) {
...
j_loop:
    for(var j=blueStart; j<blueEnd; ++j) {
      ...
      if(!testOverlap(d, axis, flip, r, b)) {
        continue
      }
  ...
  function verifyIntersect(
    ...
    function visit(i,j) {
      if(flip) {
        ...
        //Check overlap
        if(!testOverlap(d, axis, flip, red[j], blue[i])) {
