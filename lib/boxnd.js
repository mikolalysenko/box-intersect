...
function findMedian(d, axis, start, end, boxes, ids) {
  ...
  //Use quick select for first few levels
  var lo = start
  var hi = end
  var elemSize = 2*d
  var lastPivot = lo
  ...
  while(lo < hi) {
    ...
    if(hi - lo < PARTITION_THRESHOLD) {
      //For small sequences switch to insertion sort
      insertionSort(d, axis, lo, hi, boxes, ids)

      //Final scan, check mid
      var midV = boxes[elemSize*mid+axis]
      var result=mid

      //Handle degenerate case where elements are duplicated
      while(lo < result &&
          boxes[elemSize*(result-1)+axis] === midV) {
        result -= 1
      }
    ...
    lastPivot = partitionBoxes(
      d, axis, 
      lo, hi, boxes, ids,
      intervalStartLessThan,
      value2, 0.5)

    //Degenerate case: Multiple copies of mid point
    if(boxes[elemSize*lo+axis] === boxes[elemSize*hi+axis]) {
      return lo
    }

    if(mid < lastPivot) {
      hi = lastPivot - 1
    } else if(mid > lastPivot) {
      lo = lastPivot + 1
    } else {
      return lastPivot
    }
