...
//Partitioning functions - generated code is copy-pasted to avoid v8 deopts :|
...
function sweepFullIntersect(boxes, visit) {
  ...
  ndsort(ndarray(SWEEP_EVENTS, [n, 2], [2,1], 0))
...
function boxIntersectIter(
  ...
  while(top > 0) {
    ...
    //Unpack indices
    var red, redIndex, blue, blueIndex
    if(flip) {
      red       = yBoxes
      redIndex  = yIndex
      blue      = xBoxes
    ...
    var redIntervals = []
    assert(redStart < redEnd, 'red interval empty')
    for(var i=redStart; i<redEnd; ++i) {
    ...
    for(var i=blueStart; i<blueEnd; ++i) {
      ...
      assert(lo <= b0 && b0 < hi, 'blue point in interval: ' + b0 )
    ...
    */
    ...
    if(redCount  < BRUTE_FORCE_CUTOFF || 
      ...
      if(redCount > blueCount) {
        if(flip) {
          retval = bruteForceRedMajorFlip(
            d, axis, visit, 
            redStart,  redEnd,  red,  redIndex,
            blueStart, blueEnd, blue, blueIndex)
        } else {
          retval = bruteForceRedMajor(
            d, axis, visit,
            redStart,  redEnd,  red,  redIndex,
            blueStart, blueEnd, blue, blueIndex)
        }
        ...
        if(flip) {
          retval = bruteForceBlueMajorFlip(
            d, axis, visit,
            redStart,  redEnd,  red,  redIndex,
            blueStart, blueEnd, blue, blueIndex)
        } else {
          retval = bruteForceBlueMajor(
            d, axis, visit,
            redStart,  redEnd,  red,  redIndex,
            blueStart, blueEnd, blue, blueIndex)
        }
    ...
    if(Imid > redStart) {
      ...
      //TODO: handle brute force case (Imid - redStart) < BRUTE_FORCE_CUTOFF
