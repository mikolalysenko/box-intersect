...
function quickSort(left, right, data) {
  ...
  for (var k = less; k <= great; ++k) {
    if (comparePivot(k, pivot1X, pivot1Y, data) < 0) {
      if (k !== less) {
        swap(k, less, data)
      }
      ++less;
    } else {
      if (comparePivot(k, pivot2X, pivot2Y, data) > 0) {
        while (true) {
          if (comparePivot(great, pivot2X, pivot2Y, data) > 0) {
            if (--great < k) {
              break;
            }
            continue;
          } else {
            if (comparePivot(great, pivot1X, pivot1Y, data) < 0) {
